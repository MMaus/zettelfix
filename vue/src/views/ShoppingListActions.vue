<template>
  <w-flex wrap>
    ...
    <div>
      <w-button shadow @click="showDrawer = !showDrawer"
        ><w-icon lg>mdi mdi-basket-fill</w-icon></w-button
      >
    </div>
    <div>
      <w-menu hide-on-menu-click>
        <template #activator="{ on }">
          <w-button shadow v-on="on"
            ><w-icon lg>mdi mdi-storefront</w-icon></w-button
          >
        </template>
        <span class="title4">Shop wählen</span>
        <w-list
